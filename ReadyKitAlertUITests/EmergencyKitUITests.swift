...
final class EmergencyKitUITests: XCTestCase {
    ...
    func testCreateMultipleEmergencyKits() throws {
        ...
        for kitName in kitNames {
            let kitCell = app.cells.containing(.staticText, identifier: kitName).firstMatch
            XCTAssertTrue(kitCell.exists, "\(kitName) should still be visible")
        }
    }
    ...
    func testEditEmergencyKitName() throws {
        ...
        let updatedKitCell = app.cells.containing(.staticText, identifier: "Updated Name").firstMatch
        XCTAssertTrue(updatedKitCell.exists, "Updated kit name should appear")
